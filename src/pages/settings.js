...
const SettingsPage = () => {
  ...
  // Use useEffect to update the selected wallpaper whenever it changes
  useEffect(() => {
  ...
  return (
    <div className="settings-page p-6">
      ...
      <p className="text-gray-500 mb-6">
        Select a background wallpaper for your chat page:
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {wallpapers.map((wallpaper, index) => (
          <div
            key={index}
            className={`p-1 border-2 rounded-lg transition-all duration-200 ease-in-out ${
          ...
          >
            <img
              ...
              className="w-full h-32 object-cover rounded-md cursor-pointer transition-transform duration-200 ease-in-out"
            />
          </div>
        ))}
      </div>
